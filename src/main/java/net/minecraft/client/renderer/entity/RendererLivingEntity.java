...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T>
{
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        {
            if (DewCommon.moduleManager.getModule(Chams.class).isEnabled() && DewCommon.moduleManager.getModule(Chams.class).shouldRender(entity)) {
            ...
            try
            {
                ...
                if (entity instanceof EntityPlayerSP) {
                    this.inRenderPlayer((EntityPlayer) entity, f, f6, f5, f2, partialTicks);
                }
    ...
    private void inRenderPlayer(EntityPlayer realPlayer, float f, float f6, float f5, float f2, float ticks) {
        ...
        if (IMinecraft.mc.theWorld != null && IMinecraft.mc.getSession().getProfile() != null && (ghostPlayer == null || ghostPlayer.getGameProfile() != IMinecraft.mc.getSession().getProfile())) {
            ghostPlayer = new EntityOtherPlayerMP(
                    IMinecraft.mc.theWorld,
                    IMinecraft.mc.getSession().getProfile()
            );
        }
        ...
        ghostPlayer.prevRotationYaw = renderYaw;
        ghostPlayer.rotationPitch = renderPitch;
        ghostPlayer.prevRotationPitch = renderPitch;
        ghostPlayer.rotationYawHead = renderYaw;
        ghostPlayer.prevRotationYawHead = renderYaw;
        ghostPlayer.renderYawOffset = renderYaw;
        ...
        GL11.glRotatef(renderYaw - f, 0, 1, 0);
        this.mainModel.render(ghostPlayer, f6, f5, 0f, f2, renderPitch, 0.0625F);
    ...
    protected boolean setBrightness(T entitylivingbaseIn, float partialTicks, boolean combineTextures)
    {
        ...
        else
        {
            ...
            if (flag1)
            {
                ...
                if (Config.isShaders())
                {
                    Shaders.setEntityColor(1.0F, 0.0F, 0.0F, 0.6F);
                }
            }
            else
            {
    ...
    public void renderName(T entity, double x, double y, double z)
    {
        ...
        {
            if (DewCommon.moduleManager.getModule(NameTags.class).isEnabled()) {
                if (this.canRenderName(entity)) {
                    this.renderOffsetLivingLabel(entity, x, y - (entity.isChild() ? (double) (entity.height / 2.0F) : 0.0D), z, entity.getDisplayName().getFormattedText(), 0.02666667F, 0);
                } else if (DewCommon.moduleManager.getModule(NameTags.class).shouldRender(entity)) {
                    this.renderOffsetLivingLabel(entity, x, y - (entity.isChild() ? (double) (entity.height / 2.0F) : 0.0D), z, entity.getName(), 0.02666667F, 0);
                }
                return;
            }
