...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            boolean flagOrid = event.isSafeWalk && (!event.onlySafeOnGround || this.onGround) && this instanceof EntityPlayerSP;
    ...
    {
        float moveYaw = this.rotationYaw;

        if (this instanceof EntityPlayerSP) {
            StrafeEvent event = new StrafeEvent(forward, strafe, friction, moveYaw);
            DewCommon.eventManager.call(event);
            forward = event.forward;
            strafe = event.strafe;
            friction = event.friction;
            moveYaw = event.moveYaw;
            if (event.isCancelled()) return;
        }
        ...
        if (f >= 1.0E-4F)
        {
            ...
            strafe = strafe * f;
            forward = forward * f;
            float f1 = MathHelper.sin(moveYaw * (float)Math.PI / 180.0F);
            float f2 = MathHelper.cos(moveYaw * (float)Math.PI / 180.0F);
    ...
    public float getDistanceToEntityIgnoringY(Entity entityIn) {
        float dx = (float)(this.posX - entityIn.posX);
        float dz = (float)(this.posZ - entityIn.posZ);
        return MathHelper.sqrt_float(dx * dx + dz * dz);
    }
    ...
    {
        return MovePatcher.handleHitBoxSize();
    }
