...
public class MultiSelectionComponent implements ValueComponent {
    ...
    public void draw(int x, int y, int width, int mouseX, int mouseY) {
        ...
        DewCommon.customFontRenderer.drawStringWithShadow(value.getName() + " (" + value.getOptions().size() + ")", x + 4, y + 1, Color.WHITE.getRGB(), 0.35f);

        int targetHeight = isExpanded() ? 16 + value.getOptions().size() * 16 : 16;
        animatedHeight = Lerper.animate(animatedHeight, targetHeight, 0.3f);
    ...
    public void mouseClicked(int mouseX, int mouseY, int button, int x, int y) {
        if (mouseX >= x && mouseX <= x + ClickGuiState.NEW_GUI_WIDTH && mouseY >= y && mouseY <= y + 15 && button == 1) {
            toggleExpanded();
            return;
        }
